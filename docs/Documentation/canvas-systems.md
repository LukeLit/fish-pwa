...
- **`lib/game/`** – Game logic and shared state:
  ...
  - **`canvas-renderer.ts`** – `renderGame(options)`: draws background, fish, particles, and UI overlays from current state and options. When `showDepthBandOverlay` is true, draws depth band visuals (gradient zones, dotted boundaries, Level/Depth labels) in world space; see [Depth band visuals](depth-band-visuals.md).
